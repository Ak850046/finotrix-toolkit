...
(function(){
  ...
  async function extract(file,opts){
    ...
    for(let p=1;p<=pdf.numPages;p++){
      ...
      if(norm(rec.raw).length<30 && opts.ocr && window.Tesseract){
        try{if(opts.onStatus)opts.onStatus(`OCR page ${p}/${pdf.numPages}…`);const o=await ocrPage(page);if(norm(o.raw).length>norm(rec.raw).length)rec={pageNum:p,raw:o.raw,lines:o.lines};}catch(e){console.warn(e);}
      }
      pages.push(rec);
