...
(function(){
  ...
  const OPS={
    ...
    hdfc:{ label:'HDFC statement → Excel', end:true, endExt:'.xlsx',
      fields:[],
      run:async(bytes,o,ctx)=>{ if(!window.FTXHDFC||!window.XLSX) throw new Error('HDFC/Excel engine not loaded');
        const {rows,count,warn}=await FTXHDFC.parse(bytes,{onStatus:ctx.onStatus,password:o.password});
        const wb=XLSX.utils.book_new();const ws=XLSX.utils.aoa_to_sheet(rows);ws['!cols']=FTXHDFC.cols();XLSX.utils.book_append_sheet(wb,ws,'HDFC Statement');
        const out=new Uint8Array(XLSX.write(wb,{type:'array',bookType:'xlsx'}));
        return {blob:new Blob([out],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}), name:'-HDFC.xlsx', note:`${count} transactions`+(warn?' '+warn:'')};} },
  };
  window.FTXOPS=OPS;
})();
